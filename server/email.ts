...
const transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: process.env.GMAIL_USER || "gogi.reservations@gmail.com",
    pass: process.env.GMAIL_PASS || "nrjvuxmjyhvswzym",
  },
});

const FROM = '"The Gogi @ Alexandra Central" <gogi.reservations@gmail.com>';
...
export async function sendCustomerConfirmation(reservation: any): Promise<void> {
  if (!reservation.email) return; // skip if no email provided
  ...
  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 520px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; border: 1px solid #e8e4df;">
      ...
      <div style="background: #1e1a16; padding: 28px 32px; text-align: center;">
        <div style="font-size: 22px; font-weight: 700; color: #fff; letter-spacing: -0.3px;">🔥 The Gogi</div>
      ...
      <div style="padding: 32px;">
        ...
        <div style="background: #f7f5f2; border-radius: 10px; padding: 20px; margin-bottom: 24px;">
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 8px 0; color: #9a9490; font-size: 13px; width: 40%;">📅 Date</td>
              <td style="padding: 8px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${friendlyDate(reservation.date)}</td>
            </tr>
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 8px 0; color: #9a9490; font-size: 13px;">⏰ Time</td>
              <td style="padding: 8px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${reservation.time}</td>
            </tr>
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 8px 0; color: #9a9490; font-size: 13px;">👥 Party size</td>
              <td style="padding: 8px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${reservation.partySize} ${reservation.partySize === 1 ? "person" : "persons"}</td>
            </tr>
            ${reservation.notes ? `
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 8px 0; color: #9a9490; font-size: 13px;">📝 Notes</td>
              <td style="padding: 8px 0; color: #1e1a16; font-size: 13px;">${reservation.notes}</td>
            </tr>` : ""}
          </table>
        </div>
      ...
      <div style="background: #f7f5f2; padding: 16px 32px; text-align: center; border-top: 1px solid #ece9e5;">
        <p style="margin: 0; font-size: 11px; color: #b0ada8;">© The Gogi @ Alexandra Central · gogi.reservations@gmail.com</p>
      </div>
    </div>
  `;
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 520px; margin: 0 auto; background: #fff; border-radius: 12px; overflow: hidden; border: 1px solid #e8e4df;">
      ...
      <div style="padding: 28px 32px;">
        ...
        <div style="background: #f7f5f2; border-radius: 10px; padding: 20px; margin-bottom: 20px;">
          ...
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px; width: 35%;">Name</td>
              <td style="padding: 6px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${reservation.name}</td>
            </tr>
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px;">Phone</td>
              <td style="padding: 6px 0; color: #1e1a16; font-size: 13px;">+65 ${reservation.phone}</td>
            </tr>
            ${reservation.email ? `
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px;">Email</td>
              <td style="padding: 6px 0; color: #1e1a16; font-size: 13px;">${reservation.email}</td>
            </tr>` : ""}
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px;">Date</td>
              <td style="padding: 6px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${friendlyDate(reservation.date)}</td>
            </tr>
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px;">Time</td>
              <td style="padding: 6px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${reservation.time}</td>
            </tr>
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px;">Party size</td>
              <td style="padding: 6px 0; color: #1e1a16; font-size: 13px; font-weight: 600;">${reservation.partySize} pax</td>
            </tr>
            ${reservation.notes ? `
            <tr style="border-top: 1px solid #ece9e5;">
              <td style="padding: 6px 0; color: #9a9490; font-size: 13px;">Notes</td>
              <td style="padding: 6px 0; color: #c0440f; font-size: 13px; font-weight: 600;">${reservation.notes}</td>
            </tr>` : ""}
          </table>
        </div>

        <!-- Slot capacity bar -->
        <div style="background: #f7f5f2; border-radius: 10px; padding: 16px 20px;">
          ...
          <div style="display: flex; justify-content: space-between; margin-bottom: 6px;">
            ...
            <span style="font-size: 13px; color: ${fillPct >= 90 ? '#c0440f' : fillPct >= 75 ? '#d97706' : '#437a22'}; font-weight: 600;">${fillPct}% full</span>
          </div>
          <div style="background: #ece9e5; border-radius: 4px; height: 8px; overflow: hidden;">
            <div style="height: 100%; width: ${Math.min(fillPct, 100)}%; background: ${fillPct >= 90 ? '#c0440f' : fillPct >= 75 ? '#d97706' : '#437a22'}; border-radius: 4px;"></div>
          </div>
        </div>
